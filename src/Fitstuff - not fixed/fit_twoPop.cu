...
int main(int argc, char* argv[])
{
...
#if VERBOSE
  std::cerr<<"Finished initializing fitdata"<<std::endl;
#endif

  gsl_vector * params_gsl;
  params_gsl=gsl_vector_alloc(params.N);
  getGSLFromParams(&params, &priors, params_gsl);

  gsl_vector *ss;
  ss=gsl_vector_alloc(params.N);
  gsl_vector_set_all(ss, 0.25);

  gsl_multimin_function minex_func;
  minex_func.n=params.N;
  minex_func.f=g3lhalo::getChiSquared;
  minex_func.params=&fitdata;

  const gsl_multimin_fminimizer_type *T= gsl_multimin_fminimizer_nmsimplex2;
  gsl_multimin_fminimizer *s = NULL;
  s=gsl_multimin_fminimizer_alloc(T, params.N);

  gsl_multimin_fminimizer_set(s, &minex_func, params_gsl, ss);

  #if VERBOSE
  std::cerr<<"Finished initializing GSL"<<std::endl;
  #endif
    ...
    for(int i=0; i<500; i++)
  {
      ...
      status=gsl_multimin_fminimizer_iterate(s);
       ...
       //Check if simplex size less than 1e-2
	...
	{
	  std::cout<<"Converged to Minimum with chi2="<<s->fval<<std::endl;
	  break;
	};
    }

    g3lhalo::getParamsFromGSL(s->x, &priors, &params);
