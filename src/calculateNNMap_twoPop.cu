...
int main(int argc, char *argv[])
{
...
#if SCALING
  std::string fn_scaling1 = argv[23];
  std::string fn_scaling2 = argv[24];
#endif
  ...
  if (scaling1.read(Nbins, mmin, mmax, scaling1_val) || scaling2.read(Nbins, mmin, mmax, scaling2_val))
  {
    std::cerr << "Problem reading in files. Exiting." << std::endl;
    exit(1);
  };
  ...
  g3lhalo::NNMap_Model nnmap(&cosmo, zmin, zmax, kmin, kmax, mmin, mmax, Nbins,
                             ...
                             scaling1_val.data(), scaling2_val.data(),
                             &hod1, &hod2, A, epsilon);
#else
  g3lhalo::NNMap_Model nnmap(&cosmo, zmin, zmax, kmin, kmax, mmin, mmax, Nbins,
                             g_val.data(), plens1_val.data(), plens2_val.data(), w_val.data(), dwdz_val.data(),
